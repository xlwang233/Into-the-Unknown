...
def train_teaser(teaser_model, dataset, window_size, num_neg, batch_size, num_epoch, init_lr, device, logger):
    teaser_model = teaser_model.to(device)
    optimizer = torch.optim.SparseAdam(teaser_model.parameters(), lr=init_lr)
    ...
    for epoch in range(num_epoch):
        ...
        for pair_batch in next_batch(shuffle(pos_pairs), batch_size):
            ...
            if epoch_trained_batches % print_step == 0:
                # lr = init_lr * (1.0 - trained_batches / print_step)
