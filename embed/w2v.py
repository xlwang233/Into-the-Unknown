...
class HS(nn.Module):
    def __init__(self, num_vocab, embed_dimension):
        ...
        # Input embedding.
        self.u_embeddings = nn.Embedding(num_vocab, embed_dimension)
        # Output embedding. Here is actually the embedding of inner nodes.
        self.w_embeddings = nn.Embedding(num_vocab, embed_dimension, padding_idx=0)
...
class SkipGram(nn.Module):
    def __init__(self, num_vocab, embed_dimension, cbow=False):
        ...
        self.u_embeddings = nn.Embedding(num_vocab, embed_dimension)
        # Output embedding.
        self.v_embeddings = nn.Embedding(num_vocab, embed_dimension)
...
def train_skipgram(skipgram_model: SkipGram, w2v_dataset: SkipGramData,
                   window_size, num_neg, batch_size, num_epoch, init_lr, device, logger):
    skipgram_model = skipgram_model.to(device)
    optimizer = torch.optim.Adam(skipgram_model.parameters(), lr=init_lr)

    pos_pairs = w2v_dataset.gen_pos_pairs(window_size)
    # batch_count = math.ceil(num_epoch * len(pos_pairs) / batch_size)
    epoch_batch_count = math.ceil(len(pos_pairs) / batch_size)
...
def train_hs(cbow_model: HS, w2v_dataset: HSData,
    ...
    optimizer = torch.optim.Adam(cbow_model.parameters(), lr=init_lr)
