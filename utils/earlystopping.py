...
class EarlyStopping:
    ...
    def __init__(self, logger, logdir, patience=7, verbose=False, main_process=True, delta=0, run_id=None):
        """
        Args:
    ...
    def save_checkpoint(self, return_dict, model):
        ...
        if self.main_process:
            if self.run_id is not None:
                torch.save(model.state_dict(), self.log_dir + f"/checkpoint_{self.run_id}.pt")
            else:
